import flet as ft
import requests
import json
from modules import api

def main(page):
    username = ft.TextField(label="Username", autofocus=True)
    password = ft.TextField(label="Password", password=True, can_reveal_password=True)
    greetings = ft.Column()
    error_message = ft.Text("")  # Add a text widget to show the error message
    page.vertical_alignment = ft.MainAxisAlignment.CENTER
    page.horizontal_alignment = ft.CrossAxisAlignment.CENTER

    def btn_click(e):
        parse_json = api.data
        matches = [c for c in parse_json if username.value == c['name'] and password.value == c['password']]
        if matches:
            go_login(e)
        else:
            greetings.controls.append(ft.Text("GAGAL LOGIN!"))
        username.value = ""
        password.value = ""
        page.update()

    def view_pop(view):
        page.views.pop()
        top_view = page.views[-1]
        page.go(top_view.route)
    
    def create_user_view():
        # Create the view for the "User" navigation destination
        return ft.View(
            "/user",
            [
                ft.Text("User View")
            ]
        )

    def create_search_view():
        return ft.View(
            "/search",
            [
                ft.AppBar(title=ft.Text("Search"), center_title=True, bgcolor=ft.colors.SURFACE_VARIANT),
            ]
        )

    def create_bookmark_view():
        # Create the view for the "Bookmark" navigation destination
        return ft.View(
            "/bookmark",
            [
                ft.Text("Bookmark View")
            ]
        )

    def route_change(route):
        page.views.clear()
        page.views.append(
            ft.View(
                "/",
                [
                    ft.AppBar(title=ft.Text("Home"),center_title=True, bgcolor=ft.colors.SURFACE_VARIANT),
                    username,
                    password,
                    ft.Container(alignment=ft.alignment.center, content=ft.ElevatedButton("Login", on_click=btn_click)),
                    greetings,
                    error_message, 
                ],
            )
        )
        if page.route == "/login":
            user_view = create_user_view()
            search_view = create_search_view()
            bookmark_view = create_bookmark_view()
            page.views.append(
                ft.View(
                    "/login",
                    [
                        ft.AppBar(title=ft.Text("Home"), center_title=True, bgcolor=ft.colors.SURFACE_VARIANT),
                        ft.NavigationBar(
                            destinations=[
                                ft.NavigationDestination(icon=ft.icons.EXPLORE,selected_icon=ft.icons.EXPLORE_OUTLINED, label="Explore"),
                                ft.NavigationDestination(icon=ft.icons.SEARCH,selected_icon=ft.icons.SEARCH_ROUNDED, label="Search"),
                                ft.NavigationDestination(icon=ft.icons.BOOKMARK_BORDER, selected_icon=ft.icons.BOOKMARK, label="Boomark")
                            ],
                            on_change=lambda e: on_navigation_change(e, user_view, search_view, bookmark_view),
                        ),
                        ft.ElevatedButton("Go Home", on_click=lambda _: page.go("/"))
                    ]
                )
            )
        page.update()

    def go_login(e):
        page.route = "/login"
        page.update()

    def on_navigation_change(event, user_view, search_view, bookmark_view):
        selected_index = event.control.selected_index
        selected_label = event.control.destinations[selected_index].label

        # Clear the current views
        page.views.clear()

        if selected_label == "Explore":
            # Show the default view for the "Explore" destination
            page.views.append(
                ft.View(
                    "/explore",
                    [
                        ft.Text("Explore View")
                    ]
                )
            )
        elif selected_label == "Search":
            # Show the custom view for the "Search" destination
            page.views.append(search_view)
        elif selected_label == "Bookmark":
            # Show the custom view for the "Bookmark" destination
            page.views.append(bookmark_view)

        page.update()

    page.on_route_change = route_change
    page.on_view_pop = view_pop
    page.go(page.route)

ft.app(target=main)
